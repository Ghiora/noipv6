...
impl fmt::Display for UpdateError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        match self {
            NoHost =>
                f.write_str(
                    "No host was specified and login credentials do not appear to be associated with a group. Please create a group with a password to update at https://my.noip.com/#!/dynamic-dns/groups"
                ),
...
pub fn update(
    ...
    let url = match hostnames {
        Some(h) =>
            if ip.len() == 1 {
              ...
              "{}?myip={}&hostname={}",
              UPDATE_URL,
              utf8_percent_encode(&ip[0].to_string(), QUERY_SET),
              utf8_percent_encode(h.join(",").as_str(), QUERY_SET),
               ...
               UPDATE_URL,
               utf8_percent_encode(&ip[0].to_string(), QUERY_SET),
               utf8_percent_encode(&ip[1].to_string(), QUERY_SET),
               utf8_percent_encode(h.join(",").as_str(), QUERY_SET)
            )
        }
        
        // Why is there code in None??
        None =>
            format!(
            "{}?myip={}",
            UPDATE_URL,
    ...
    let body = r.as_str().map_err(|e| UpdateError::Unknown(format!("{}", e)))?;

    match r.status_code {
        200 => {}
        401 => {
            return Err(UpdateError::BadAuth);
        }
        _ => {
            return Err(UpdateError::StatusCode(r.status_code, r.reason_phrase.clone()));
        }
    }
